...
function ReportsPage() {
      ...
      render: (text, record) => (
        <>
          {record.result.correctAnswers.length *
            (record.exam.totalMarks / record.exam.questions.length)}
        </>
      ),
    },
    {
  ...
  const getData = async () => {
    try {
      ...
      if (response.success) {
        console.log(
          response.data[0].exam.totalMarks /
            response.data[0].exam.questions.length
        );
        console.log(response.data[0].result.correctAnswers.length);
